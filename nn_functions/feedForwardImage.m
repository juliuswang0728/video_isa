...
X = reshape(X, [dFovea_spSize, dFovea_spSize, 3, nPatches]);
for it = 0: nSubSample - 1
    [x, y] = ind2sub([nSubSample_per_spCol_dL, nSubSample_per_spCol_dL], it+1);
    startx = (x-1)*spatial_stride + 1;
    starty = (y-1)*spatial_stride + 1;
    endx = startx + sFovea_spSize - 1;
    endy = starty + sFovea_spSize - 1;
    subFoveaX = reshape(X(startx:endx, starty:endy, :, :), nFoveaSRC, nPatches);
